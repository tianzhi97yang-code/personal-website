...
import { Search, Tag, Calendar, MapPin, ExternalLink, Download, Plus, Trash2, Save, X, Edit2 } from 'lucide-react';
...
const HomePage = ({ language, memes, addMeme, deleteMeme, isAdmin }: { language: Language, memes: MemePost[], addMeme: (m: any) => void, deleteMeme: (id: string) => void, isAdmin: boolean }) => {
  const [newMeme, setNewMeme] = useState({ title: '', caption: '', imageUrl: '' });
  const [isAdding, setIsAdding] = useState(false);

  const handleAdd = () => {
    if (!newMeme.imageUrl) return;
    addMeme({ ...newMeme }); // ID generated by DB or removed
    setNewMeme({ title: '', caption: '', imageUrl: '' });
    setIsAdding(false);
  };

  const handleDelete = (id: string) => {
    if (window.confirm("Delete this poster?")) deleteMeme(id);
  };
            ...
            <input type="file" accept="image/*" onChange={async e => {
              if (e.target.files?.[0]) {
                try {
                  ...
                  setNewMeme({ ...newMeme, imageUrl: processed });
                } catch (error) {
...
const CVPage = ({ language, profile, updateProfile, education, addEducation, deleteEducation, updateEducation, isAdmin }:
  { language: Language, profile: UserProfile, updateProfile: (p: UserProfile) => void, education: EducationItem[], addEducation: (e: any) => void, deleteEducation: (id: string) => void, updateEducation: (id: string, e: any) => void, isAdmin: boolean }) => {

  const [isEditingProfile, setIsEditingProfile] = useState(false);
  const [tempProfile, setTempProfile] = useState(profile);

  // Sync temp profile when real profile updates (e.g. initial load)
  useEffect(() => {
  ...
  const deleteEdu = (id: string) => {
    if (window.confirm("Remove this education item?")) deleteEducation(id);
  };

  const addEdu = () => {
    const newEdu: EducationItem = {
      id: Date.now().toString(), // Helper ID, replaced by DB
      degree: "New Degree",
      school: "New School",
      year: "2024",
      color: "bg-rose-200"
  ...
  // Helper for updating edu fields locally before save? 
  // Education is array, updating one item logic:
  // With Supabase, we should probably have inline save or 'save' button per item, OR optimistic update.
  // existing logic was local state update. 
  // Let's modify handlers to call updateEducation directly (which includes optimistic update)
  const handleEduChange = (id: string, field: keyof EducationItem, value: string) => {
  ...
  const [cropCallback, setCropCallback] = useState<(img: string) => void>(() => { });
            ...
            <div className="space-y-3 w-full bg-stone-50 dark:bg-stone-900/50 p-6 rounded-2xl border border-stone-100 dark:border-stone-700">
              ...
              <div className="flex items-center gap-2 mb-4">
                ...
                <input type="file" accept="image/*" onChange={async e => {
                  if (e.target.files?.[0]) {
                    try {
                      // Just optimize cover image, no crop
                      const processed = await processImage(e.target.files[0], 1920);
                      setTempProfile({ ...tempProfile, coverImage: processed });
                    } catch (err) {
                ...
                <input type="file" accept="image/*" onChange={e => {
                  if (e.target.files?.[0]) {
                    ...
                    reader.onload = () => {
                      ...
                      setCropCallback(() => (cropped: string) => {
                        setTempProfile(prev => ({ ...prev, avatarUrl: cropped }));
                      });
                    };
              ...
              {isAdmin ? (
                <div className="flex items-start gap-2">
                  <div className="flex-1 grid gap-2">
                    <input value={edu.degree} onChange={(e) => handleEduChange(edu.id, 'degree', e.target.value)} className="font-bold text-lg bg-transparent border-b border-stone-200 dark:border-stone-700 outline-none dark:text-stone-200" />
                    <input value={edu.school} onChange={(e) => handleEduChange(edu.id, 'school', e.target.value)} className="text-soft-brown bg-transparent border-b border-stone-200 dark:border-stone-700 outline-none" />
                    <input value={edu.year} onChange={(e) => handleEduChange(edu.id, 'year', e.target.value)} className="text-xs text-stone-400 bg-transparent outline-none" />
                  </div>
...
const PublicationsPage = ({ language, publications, addPublication, updatePublication, deletePublication, isAdmin }:
  { language: Language, publications: Publication[], addPublication: (p: any) => void, updatePublication: (id: string, f: string, v: any) => void, deletePublication: (id: string) => void, isAdmin: boolean }) => {

  const addPub = () => {
    const newPub: Publication = { id: '', title: "New Paper Title", authors: "Authors List", venue: "Conference/Journal", year: new Date().getFullYear() };
      ...
      <div className="grid gap-4">
        {publications.map((pub) => (
          <div key={pub.id} className="group p-6 bg-white dark:bg-stone-800 rounded-2xl border border-transparent hover:border-rose-200 dark:hover:border-stone-600 transition-all shadow-sm relative">
            {isAdmin && (
              <button onClick={() => { if (window.confirm("Delete?")) deletePublication(pub.id); }} className="absolute top-4 right-4 text-stone-300 hover:text-red-500"><Trash2 size={16} /></button>
            )}
            {isAdmin ? (
              <div className="grid gap-2 pr-8">
                <input value={pub.title} onChange={e => updatePublication(pub.id, 'title', e.target.value)} className="font-bold text-lg bg-transparent border-b border-dashed border-stone-300 w-full outline-none dark:text-stone-200" />
                <input value={pub.authors} onChange={e => updatePublication(pub.id, 'authors', e.target.value)} className="text-sm italic bg-transparent border-b border-dashed border-stone-300 w-full outline-none dark:text-stone-400" />
                <div className="flex gap-4">
                  <input value={pub.venue} onChange={e => updatePublication(pub.id, 'venue', e.target.value)} className="text-xs font-bold uppercase bg-transparent border-b border-dashed border-stone-300 outline-none dark:text-rose-400 text-rose-400" />
                  <input type="number" value={pub.year} onChange={e => updatePublication(pub.id, 'year', parseInt(e.target.value))} className="text-xs font-bold bg-transparent border-b border-dashed border-stone-300 w-20 outline-none dark:text-stone-400" />
                </div>
              </div>
            ) : (
              <>
...
const ConferencesPage = ({ language, conferences, addConference, updateConference, deleteConference, isAdmin }:
  { language: Language, conferences: Conference[], addConference: (c: any) => void, updateConference: (id: string, f: string, v: any) => void, deleteConference: (id: string) => void, isAdmin: boolean }) => {

  const addConf = () => {
    addConference({ title: "New Talk", event: "Conference Name", date: "Date", location: "Location" });
  };
        ...
        {conferences.map((conf) => (
          <div key={conf.id} className="relative pl-8 group">
            ...
            <div className="bg-white/50 dark:bg-stone-800/50 p-6 rounded-2xl backdrop-blur-sm hover:bg-white dark:hover:bg-stone-800 transition-colors relative">
              {isAdmin && <button onClick={() => { if (window.confirm("Delete?")) deleteConference(conf.id); }} className="absolute top-4 right-4 text-stone-300 hover:text-red-500"><Trash2 size={16} /></button>}

              {isAdmin ? (
                <div className="grid gap-2">
                  <input value={conf.date} onChange={e => updateConference(conf.id, 'date', e.target.value)} className="text-xs font-bold text-rose-500 bg-transparent border-b border-stone-300 w-full outline-none" />
                  <input value={conf.title} onChange={e => updateConference(conf.id, 'title', e.target.value)} className="text-xl font-serif font-bold dark:text-stone-200 bg-transparent border-b border-stone-300 w-full outline-none" />
                  <div className="flex gap-2">
                    <input value={conf.event} onChange={e => updateConference(conf.id, 'event', e.target.value)} className="text-sm font-bold bg-transparent border-b border-stone-300 outline-none flex-1 dark:text-stone-400" />
                    <input value={conf.location} onChange={e => updateConference(conf.id, 'location', e.target.value)} className="text-sm bg-transparent border-b border-stone-300 outline-none flex-1 dark:text-stone-400" />
                  </div>
                </div>
              ) : (
                <>
...
const BlogPage = ({ language, posts, addBlogPost, deleteBlogPost, isAdmin }: { language: Language, posts: BlogPost[], addBlogPost: (p: any) => void, deleteBlogPost: (id: string) => void, isAdmin: boolean }) => {
  const [activeCategory, setActiveCategory] = useState('all');
  const [isWriting, setIsWriting] = useState(false);
  const [newPost, setNewPost] = useState<Partial<BlogPost>>({ title: '', content: '', category: 'essays', tags: [], excerpt: '' });

  // Keys correspond to TRANSLATIONS
  const categories = ['all', 'research', 'essays', 'trash'];
  ...
  const handleSavePost = () => {
    ...
    const post = {
      ...
      category: newPost.category || "essays",
    ...
    addBlogPost(post);
    setIsWriting(false);
    setNewPost({ title: '', content: '', category: 'essays', tags: [], excerpt: '' });
  };

  const handleDelete = (id: string) => {
    if (window.confirm("Delete post?")) deleteBlogPost(id);
  };

  // Helper to get translated name
  const getCatName = (key: string) => {
    // @ts-ignore
    return TRANSLATIONS[language].blog.categories[key] || key;
  };
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-4">
            <select value={newPost.category} onChange={e => setNewPost({ ...newPost, category: e.target.value })} className="p-2 rounded-lg bg-warm-paper dark:bg-stone-900 dark:text-stone-300 outline-none">
              {categories.filter(c => c !== 'all').map(c => <option key={c} value={c}>{getCatName(c)}</option>)}
            </select>
...
const App: React.FC = () => {
  ...
  const {
    ...
    memes, addMeme, deleteMeme,
    publications, addPublication, updatePublication, deletePublication,
    conferences, addConference, updateConference, deleteConference,
  ...
  if (loading) {
    return (
      <div className="w-screen h-screen flex items-center justify-center bg-warm-cream dark:bg-stone-900">
        <div className="text-center">
          <div className="text-4xl animate-bounce mb-4">🌿</div>
          <p className="font-serif text-deep-brown dark:text-stone-300">Loading your cozy space...</p>
        </div>
      </div>
    );
  }
    ...
    <HashRouter>
      <Routes>
        ...
        <Route path="/app" element={
          ...
          <Route path="home" element={<HomePage language={language} memes={memes} addMeme={addMeme} deleteMeme={deleteMeme} isAdmin={isAdmin} />} />
          <Route path="cv" element={<CVPage language={language} profile={profile} updateProfile={updateProfile} education={education} addEducation={addEducation} updateEducation={updateEducation} deleteEducation={deleteEducation} isAdmin={isAdmin} />} />
          <Route path="publications" element={<PublicationsPage language={language} publications={publications} addPublication={addPublication} updatePublication={updatePublication} deletePublication={deletePublication} isAdmin={isAdmin} />} />
          <Route path="conferences" element={<ConferencesPage language={language} conferences={conferences} addConference={addConference} updateConference={updateConference} deleteConference={deleteConference} isAdmin={isAdmin} />} />
          <Route path="blog" element={<BlogPage language={language} posts={blogPosts} addBlogPost={addBlogPost} deleteBlogPost={deleteBlogPost} isAdmin={isAdmin} />} />
          <Route path="blog/:id" element={<BlogDetailPage language={language} posts={blogPosts} />} />
          <Route path="analytics" element={isAdmin ? <AnalyticsPage /> : <Navigate to="home" />} />
